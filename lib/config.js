...
function getConfig(options) {
  ...
  var css2js = require('grunt-cmd-transport-xd').css2js;
  ...
  Object.keys(output).forEach(function(name) {
    ...
    jsconcats = [
      ...
      // .build/src/xxx-{hash}.js => .build/tmp/xxx-{hash}.js
      {
        ...
        src: ['**/*.js'],
        filter: function(src) {
          if (matchDebug(src)) return false;
          var m = src.match(/^\.build\/src\/(.*?)-[a-z0-9]{8}((?:\.[a-z]*)?\.js)$/);
          if (!m) {
            return false;
          }
          return typeof output[m[1] + m[2]] !== 'undefined';
        },
        dest: '.build/tmp'
      },
      // .build/src/xxx-{hash}-debug.js => .build/dist/xxx-{hash}-debug.js
      {
        expand: true,
        cwd: '.build/src',
        src: '**/*.js',
        filter: function(src) {
          if (!matchDebug(src)) return false;
          var m = src.match(/^\.build\/src\/(.*?)-[a-z0-9]{8}-debug((?:\.[a-z]*)?\.js)$/);
          if (!m) {
            return false;
          }
          return typeof output[m[1] + m[2]] !== 'undefined';
    ...
    cssmins = [{
      ...
      filter: function(src) {
        if (matchDebug(src)) return false;
        var m = src.match(/^\.build\/tmp\/(.*?)-[a-z0-9]{8}(\.css)$/);
        if (!m) {
          return false;
        }
        return typeof output[m[1] + m[2]] !== 'undefined';
    ...
    copies = [
      // .build/tmp/xxx-{hash}-debug.css => .build/dist/xxx-{hash}-debug.css
      {
        expand: true,
        cwd: '.build/tmp',
        src: '**/*.css',
        filter: function(src) {
          if (!matchDebug(src)) return false;
          var m = src.match(/^\.build\/tmp\/(.*?)-[a-z0-9]{8}-debug(\.css)$/);
          if (!m) {
            return false;
          }
          return typeof output[m[1] + m[2]] !== 'undefined';
  ...
  return {
    ...
    cssmin: {
      options: {
        ...
        banner: '/*! build date: <%= grunt.template.today("yyyy-mm-dd HH:MM")%> by <%=process.env["USER"]%> */'
      },
      css: {files: cssmins}
    },
    uglify: {
...
function matchDebug(filepath) {
  return /-debug(\.[a-z]*)?\.(js|css)$/.test(filepath);
}
